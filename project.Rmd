...
$$g(\theta^{(i)})=\frac{\partial Q(\theta|{\theta}^{(i)})}{\partial \theta}|_{\theta=\theta^{(i)}}=-(159-\frac{250}{2+\theta^{(i)}})\frac{1}{{\theta^{(i)}}^2}+\frac{250}{\theta^{(i)}(2+\theta^{(i)})^2}-\frac{38}{(1-\theta^{(i)})^2}$$
...
print(paste("iternation=",iter_EMNR))
...
# 4 Result
```{r}
table=rbind(c(theta_EM[length(theta_EM)],theta_MCEM[length(theta_MCEM)],theta_EMNR[length(theta_EMNR)],theta_SQUAREM[[1]]),c(iter_EM,iter_MCEM,iter_EMNR,iter_SQUAREM),c(t2-t1,t4-t3,t6-t5,t8-t7))
